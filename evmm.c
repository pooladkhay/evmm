...
static int evmm_cpu_set_cr0_cr4(struct evmm_percpu_config *conf)
{
	...
	/* set VMXE only if it's not already set */
	if (!(conf->orig_cr4 & X86_CR4_VMXE)) {
		// cr4_set_bits(X86_CR4_VMXE);
		// conf->vmxe = true;
		pr_alert("evmm: VMXE already set\n");
	}
	...
	// might not be required if fixed MSRs already cover this bit.
	cr4 |= X86_CR4_VMXE;
...
static int evmm_cpu_enter_vmx(struct evmm_percpu_config *conf)
{
	...
	if (asm_err) {
		pr_err("evmm: 'vmxon' failed with %d.\n", asm_err);
		return -EIO;
	}
	conf->vmxon = true;
